                ...
                if self.playlist_url.get() == "":
                        ...
                        try:
                                ...
                                for self.video in self.playlist:
                                        self.video.streams.get_highest_resolution().download(output_path=self.path,
                                                                                             filename=None)

                                self.progress = Progressbar(self.root, orient=HORIZONTAL, length=250,
                                                            mode='determinate')
                                self.progress['value'] = 20
                                self.root.update_idletasks()
                                self.progress['value'] = 40
                                ...
                                self.dow_details = ScrolledText(self.root, width=30, height=3,
                                                                font=('verdana', 8, 'bold'))
                                self.dow_details.place(x=20, y=70)
